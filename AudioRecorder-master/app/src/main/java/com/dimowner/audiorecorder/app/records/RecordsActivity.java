...
import com.dimowner.audiorecorder.debug.Energy;
import com.dimowner.audiorecorder.debug.MaxFreq;
		...
		adapter.setOnItemOptionListener(new RecordsAdapter.OnItemOptionListener() {
			...
			public void onItemOptionSelected(int menuId, final ListItem item) {
				switch (menuId) {
					...
					String file_path = presenter.getActiveRecordPath();
					if (file_path!=null) {
						//Analyse.display_hr(file_path);
						...
						//builder.setTitle(R.string.warning)
                                ...
                                for (int i=0;i<10;i++){
									indices[i] = (int)Energy.main(item.getPath())[i+1];
								}
								builder.setMessage("\n Heart Rate = "+(int)HeartRate.main(item.getPath()) +" BPM" +"\n Max Energy = "+(float)Energy.main(item.getPath())[0] +"\n Max Frequency = "+MaxFreq.main(item.getPath(), indices)+ " Hz"); }
                                //builder.setMessage(Double.toString(Energy.main(item.getPath())));}
                                catch(IOException e){
